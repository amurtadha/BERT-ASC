...
def sentihood_macro_F1(y_true, y_pred):
    ...
    if (p + r) == 0:
        ...
        f = 2 * p * r / (p + r)
    #
    # print('Aspect Micro F1 {}'.format(round(f,4)))
...
def sentihood_AUC_Acc(y_true, score):
    ...
    for i in range(len(y_true)):
        if y_true[i] > 0:
            ...
            # sentiment_y_pred.append(np.argmax(score[i][1:]))
            # sentiment_y_score.append(score[i][2])
...
def semeval_PRF(y_true, y_pred):
    ...
    for i in range(len(y_pred) // 5):
        s = set()
        g = set()
        # al_t.extend(y_true[i:i+5])
        # al_p.extend(y_pred[i:i+5])
        for j in range(5):
            if y_pred[i * 5 + j] != 4:
                s.add(j)
            if y_true[i * 5 + j] != 4:
...
def semeval_PRF_each_aspect(y_true, y_pred):
    ...
    # print(metrics.classification_report(al_t,al_p))
...
def _semeval_PRF(y_true, y_pred):
    ...
    # rec, pre, f1=[],[],[]
    for i in range(len(y_pred) // 5):
        # y= [0 if y == 0 else 1 for y in y_true[i:i + 5]]
        # p= [0 if y == 0 else 1 for y in y_pred[i:i + 5]]
        # rec.append(recall_score(y, p, average='macro'))
        # pre.append(precision_score(y, p, average='macro'))
        # f1.append(f1_score(y, p, average='macro'))
        s = set()
        g = set()
        for j in range(5):
            if y_pred[i * 5 + j] != 4:
    ...
    if (p + r) == 0:
        ...
        f = 2 * p * r / (p + r)
    # print(Ma_p, Ma_r, aspect_Macro_F1)
    # return p, r, f
    #
    # print(np.mean(pre),np.mean(rec),np.mean(f1))
